...
parseGame = parse gameParser ""
  where
    ...
    playerParser n = string ("Player " ++ n ++ ":") *> newline *> (Seq.fromList <$> intParser `sepEndBy1` newline)
...
winningScore (Empty, scores) = scores & Seq.reverse & toList & zipWith (*) [1 ..] & sum
winningScore (scores, Empty) = scores & Seq.reverse & toList & zipWith (*) [1 ..] & sum
